...
namespace PBL3.Service
{
    public class ProfitService : IProfitService
    {
        ...
        public ProfitService(MilkTeaDBContext conn)
        {
            _conn = conn;
        }
        ...
        public double GetProfitInRange(DateTime startDate, DateTime endDate)
        {
            ...
            var profit = (from o in _conn.Orders
                          join od in _conn.OrderDetails on o.orderID equals od.orderID
        ...
        public int CalculateTotalIngredientCost(DateTime start, DateTime end)
        {
            ...
            var totalcost = from o in _conn.Orders
                            where o.orderStatus == "Completed"
                                  && o.orderDate >= start.Date
                                  && o.orderDate <= endOfDay

                            join od in _conn.OrderDetails on o.orderID equals od.orderID
                            join r in _conn.Recipes on new { id = od.itemID, size = od.size } equals new { id = r.itemID, size = r.size }
                            join ig in _conn.Ingredients on r.ingredientID equals ig.igID
